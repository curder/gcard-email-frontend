    ...
    <div class="flex items-center justify-center">
        <div class="w-full">
            <div class="flex justify-between items-center">
                <h2 class="text-xl text-gray-800">邮箱列表<span class="text-sm text-gray-700 ml-1">获取引导邮件</span></h2>
                <div>
                    <select v-model="form.code"
                        ...
                        <option v-for="(text, code) in statusList" :value="code" v-text="code + text"
                                :key="code"></option>
                    </select>
                    ...
                    <select name="trashed"
                            ...
                            :class="selectClass"
                            class="form-select py-1">
                        <option :value="trashed.value" v-text="trashed.label" v-for="(trashed, key) in trashedList"
                                :key="key"></option>
                    </select>
                </div>
            </div>
                    ...
                    <tr class="hover:bg-gray-100 text-gray-700"
                        ...
                        <td class="p-2 border border-gray-300">
                            <span class="inline-block rounded-full w-2 h-2"
                                  :class="{'bg-green-500': item.status, 'bg-red-500': !item.status}"></span>
                        ...
                        <td class="p-2 border border-gray-300">
                            <a class="text-red-400 hover:text-red-500 hover:underline"
                               href="javascript:void(0);"
                               v-if="!item.is_delete"
                               @click.prevent="destroy(item.id)">删除</a>
